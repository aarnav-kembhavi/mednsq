"""
MedNSQ full experiment pipeline for Phi-3.
...
CRITICAL: This script intentionally adapts to the existing modules WITHOUT modifying them:
  ...
  - notebooks/mednsq_probe.py (not imported; we re-implement Phi-3-specific EMS core here)

Target model:
  microsoft/Phi-3-mini-128k-instruct

Architecture assumptions (STRICT, Phi-3 only):
  - layers = model.model.layers
  - mlp down_proj = layer.mlp.down_proj
  - attention output = layer.self_attn.o_proj
"""
...
def _phi3_o_proj_weight(model, layer_idx: int) -> torch.Tensor:
    layers = _get_phi3_layers(model)
    layer = layers[layer_idx]
    if not hasattr(layer, "self_attn") or not hasattr(layer.self_attn, "o_proj"):
        raise RuntimeError(
            f"Phi-3 architecture check failed at layer {layer_idx}: "
            "expected layer.self_attn.o_proj"
        )
...
def _answer_position_single(input_ids: torch.Tensor, letter_token_ids: torch.Tensor) -> int:
    """Find the last position in the sequence where token is one of A/B/C/D."""
    if input_ids.dim() == 2:
    ...
    if not positions:
        raise RuntimeError("No answer token found in sequence")
...
def _answer_positions_batch(
    ...
    Returns list of answer_idx per sample. Raises if any sample has no such token.
    """
    letter_set = set(int(x) for x in letter_token_ids.cpu().tolist())
    result: List[int] = []
...
def evaluate_column_ems_phi3(
    ...
    drops = baseline_margins[: crushed_tensor.numel()] - crushed_tensor
...
def sanity_check_top_vs_random(
    ...
    """Compare mean_drop of top-5 Taylor columns vs 5 random columns on a small subset."""
    ...
    top5_cols = top_indices[:5].tolist()
    random_cols = torch.randperm(num_cols)[:5].tolist()
...
def run_random_neuron_ablation_baseline_phi3(
    ...
    w0 = _phi3_down_proj_weight(model, int(layers[0]))
...
def _run_forward_no_hooks(model, input_ids: torch.Tensor, attention_mask: torch.Tensor) -> torch.Tensor:
    with torch.no_grad():
        out = model(input_ids=input_ids, attention_mask=attention_mask)
    return out.logits[:, -1, :]
...
def run_anchor_activation_patching_experiment_phi3(
    ...
    def _run_with_save_hooks(stored: Dict[Tuple[int, int], torch.Tensor], input_ids, attention_mask) -> torch.Tensor:
        handles = []
        for layer_idx, cols in anchors_by_layer.items():
            down_proj = layers[layer_idx].mlp.down_proj

            def _save_hook(module, inputs, layer=layer_idx, cols_list=cols):
                h = inputs[0]
                # Save final-token activations for requested columns.
    ...
    def _run_with_patch_hooks(stored: Dict[Tuple[int, int], torch.Tensor], input_ids, attention_mask) -> torch.Tensor:
        handles = []
        for layer_idx, cols in anchors_by_layer.items():
            down_proj = layers[layer_idx].mlp.down_proj

            def _patch_hook(module, inputs, layer=layer_idx, cols_list=cols):
                h = inputs[0]
                out = h.clone()
                for col in cols_list:
                    if (layer, col) in stored:
                        out[0, -1, col] = stored[(layer, col)].to(dtype=out.dtype, device=out.device)
                return (out,)
    ...
    with torch.no_grad():
        for i in range(n_samples):
            ...
            _run_with_save_hooks(stored, safe_ids_pad, safe_mask_pad)

            logits_base = _run_forward_no_hooks(model, corrupt_ids_pad, corrupt_mask_pad)
    ...
    def _run_with_patch_hooks_random(stored: Dict[Tuple[int, int], torch.Tensor], input_ids, attention_mask) -> torch.Tensor:
        handles = []
        for layer_idx, cols in random_by_layer.items():
            down_proj = layers[layer_idx].mlp.down_proj
...
def run_attention_head_ablation_sweep_phi3(
    ...
    sweep_results: Dict[int, Dict[str, List[float]]] = {k: {"accuracies": [], "margins": []} for k in k_values}
    layer_stack = _get_phi3_layers(model)
...
def run_head_to_anchor_attribution_phi3(
    ...
    def _run_and_capture_anchors(store: Dict[Tuple[int, int], List[torch.Tensor]]) -> None:
        ...
        for layer_idx, cols in anchors_by_layer.items():
            ...
            def _capture(module, inputs, layer=layer_idx, anchor_cols=cols):
                h = inputs[0]
                for col in anchor_cols:
                    store[(layer, col)].append(h[:, -1, col].detach().cpu())

            handles.append(down_proj.register_forward_pre_hook(_capture))
        try:
            with torch.no_grad():
...
model.eval()
    ...
    w0 = _phi3_down_proj_weight(model, 0)
    print("[Sanity] Layer 0 down_proj.weight shape:", tuple(w0.shape))
    ...
    # Random neuron ablation baseline.
    anchor_layers = sorted(list(set(int(layer) for layer, _ in anchors))) or [int(LAYERS_TO_TEST[0])]
