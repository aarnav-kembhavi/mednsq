...
# Save final anchor list to anchors_gemma_base.json and append to anchors_progress.txt
        anchors_final_data = [
            {"layer": layer, "column": col, "drop": stats["drop"], "z": stats["z"]}
            for (layer, col), stats in sorted_anchor_stats
        ]
        with open("anchors_gemma_base.json", "w") as f:
            json.dump(anchors_final_data, f, indent=2)
        print("Saved anchor list to anchors_gemma_base.json")
        log_progress("Saved anchor list to anchors_gemma_base.json")
        with open("anchors_progress.txt", "a") as f:
